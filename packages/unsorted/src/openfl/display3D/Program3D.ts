...
import Context3DProgramFormat from "./Context3DProgramFormat";
...
/**
	...
	method instead.
**/
export default class Program3D
{
	protected __context: Context3D;
	protected __format: Context3DProgramFormat;
	...
	protected constructor(context3D: Context3D, format: Context3DProgramFormat)
	{
		...
		this.__samplerStates = new Array<SamplerState>();
	...
	**/
	public getAttributeIndex(name: string): number
	{
	...
	/**
		**BETA**
		...
		was not found in the shader sources
	**/
	public getConstantIndex(name: string): number
	{
		switch (this.__format)
		{
			case Context3DProgramFormat.AGAL:
	...
	**/
	...
	protected __getSamplerState(sampler: number): SamplerState
	{
		return this.__samplerStates[sampler];
	}

	protected __markDirty(isVertex: boolean, index: number, count: number): void
	{
