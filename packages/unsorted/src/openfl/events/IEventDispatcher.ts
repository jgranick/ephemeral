...
export default interface IEventDispatcher
{
	/**
		...
		@param	listener	The listener that processes the event. This function
		must accept an event object as its only parameter and must return nothing, as
		this example shows:
		```haxe
		function(evt:Event):Void
		```
		The can have any name.
		...
		that is a nested inner function, the will be garbage-collected and no
		longer be persistent. If you create references to the inner (save it in
		another variable) then it is not garbage-collected and stays persistent.
	**/
	addEventListener<T>(type: EventType<T>, listener: (event: T) => void, useCapture?: boolean, priority?: number, useWeakReference?: boolean): void;
	...
	**/
	removeEventListener<T>(type: EventType<T>, listener: (event: T) => void, useCapture?: boolean): void;
